...
import { TimeframeAnalyzer } from '../lib/einstein/data/timeframeAnalyzer';
import type { TrendDirection, TimeframeAlignment } from '../lib/einstein/types';
...
describe('TimeframeAnalyzer', () => {
  ...
  describe('Trend Determination', () => {
    it('should determine BULLISH trend for uptrending prices', async () => {
      const uptrendPrices = Array.from({ length: 50 }, (_, i) => 100 + i * 2);
      const indicators = await (analyzer as any).calculateTimeframeIndicators(uptrendPrices, '1h');
      const trend = (analyzer as any).determineTrend(indicators, uptrendPrices);
      
      expect(trend).toBe('BULLISH');
    });

    it('should determine BEARISH trend for downtrending prices', async () => {
      const downtrendPrices = Array.from({ length: 50 }, (_, i) => 200 - i * 2);
      const indicators = await (analyzer as any).calculateTimeframeIndicators(downtrendPrices, '1h');
      const trend = (analyzer as any).determineTrend(indicators, downtrendPrices);
      
      expect(trend).toBe('BEARISH');
    });

    it('should determine NEUTRAL trend for sideways prices', async () => {
      const sidewaysPrices = Array.from({ length: 50 }, (_, i) => 100 + (i % 2 === 0 ? 1 : -1));
      const indicators = await (analyzer as any).calculateTimeframeIndicators(sidewaysPrices, '1h');
