...
describe('Confidence Score Calculator', () => {
  ...
  describe('calculateVeritasConfidenceScore', () => {
    ...
    test('handles empty validation results', () => {
      ...
      // With no data sources, completeness is 0 (10% weight)
      // But logical consistency is 100 (30% weight) since no fatal errors
      // So overall score = 0*0.4 + 100*0.3 + 0*0.2 + 0*0.1 = 30
      expect(score.overallScore).toBe(30);
      expect(score.completeness).toBe(0);
      expect(score.dataSourceAgreement).toBe(0);
      expect(score.logicalConsistency).toBe(100);
      expect(score.crossValidationSuccess).toBe(0);
    });
