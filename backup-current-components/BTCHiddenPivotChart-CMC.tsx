...
export default function BTCHiddenPivotChart() {
  ...
  const fetchPivotData = async (tf: string) => {
    ...
    try {
      console.log(`🔍 Fetching BTC hidden pivot analysis from CoinMarketCap (${tf})`);
      
      const response = await fetch(`/api/cmc-trading-analysis?symbol=BTC&timeframe=${tf}`);
      const result = await response.json();

      if (result.success || result.data.length > 0) {
        console.log(`✅ BTC hidden pivot analysis received:`, {
      ...
      console.error('❌ BTC hidden pivot analysis failed:', err);
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-3 text-gray-600">Analyzing BTC hidden pivots via CoinMarketCap...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex items-center justify-center h-64 text-red-600">
          <span>Error loading BTC pivot data: {error}</span>
          <button 
  ...
  const currentPrice = pivotData.data.length > 0 ? 
    pivotData.data[pivotData.data.length - 1].close : 110000;
