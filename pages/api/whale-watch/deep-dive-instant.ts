 ...
 * 
 * Uses GPT-5.1 (o1-mini) for comprehensive whale transaction analysis
 * This is the endpoint called by the "ChatGPT 5.1 (Latest)" button
 * 
...
export default async function handler(
  ...
  try {
    console.log(`🚀 Instant Deep Dive API called (GPT-5.1)`);
...
Be specific with numbers and actionable recommendations.`;
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout') || error.message.includes('abort')) {
        ...
        errorDetails = 'The analysis took too long. Try again.';
