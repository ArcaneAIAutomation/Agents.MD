...
async function fetchRealBitcoinData() {
  ...
  try {
    // 5. Get recent Bitcoin news from NewsAPI
    if (process.env.NEWS_API_KEY && process.env.NEWS_API_KEY !== 'undefined') {
      const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
      const newsResponse = await fetch(`https://newsapi.org/v2/everything?q=bitcoin&from=${yesterday}&sortBy=publishedAt&pageSize=5&language=en&apiKey=${process.env.NEWS_API_KEY}`, {
        signal: AbortSignal.timeout(5000)
      });
      ...
      if (newsResponse.ok) {
        ...
        if (newsData.articles && newsData.articles.length > 0) {
          ...
          console.log('✅ News data:', results.newsData.length, 'articles');
        }
      }
    }
  } catch (error) {
...
function calculateTechnicalIndicators(currentPrice: number, high24h: number, low24h: number, volume24h: number, orderBookData: any) {
  ...
  const supplyDemandAnalysis = analyzeSupplyDemandZones(orderBookData, currentPrice);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    console.log('🚀 Starting 100% REAL Bitcoin analysis with OpenAI GPT-4o...');
    ...
    if (!realData.price?.current) {
      throw new Error('Failed to fetch real Bitcoin price data');
    }
      ...
      try {
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: `You are an expert Bitcoin trader and technical analyst. Analyze the provided REAL market data and generate professional trading insights.
              
              REAL MARKET DATA:
              - Current Price: $${currentPrice.toLocaleString()}
              - 24h Change: ${realData.price.change24h}%
              - 24h Volume: ${realData.price.volume24h.toLocaleString()} BTC
              ...
              {
                "analysis": "Professional market analysis in 2-3 sentences",
                "sentiment": "Bullish",
                "predictions": {
                  "hourly": {"target": 117000, "confidence": 75},
                  "daily": {"target": 118000, "confidence": 70},
                  "weekly": {"target": 120000, "confidence": 65}
                },
                "tradingSignals": [
                  {"signal": "BUY", "strength": "Medium", "timeframe": "1H", "confidence": 75}
                ],
                "keyLevels": {
                  "support": [115000, 114000],
                  "resistance": [117000, 118000]
                }
              }
            ...
            {
              role: "user",
              content: `Analyze this REAL Bitcoin market data and provide professional trading insights.`
            }
          ],
          temperature: 0.3,
          max_tokens: 800
        ...
        if (aiContent) {
          try {
            ...
            aiAnalysis = aiResult.analysis || 'Bitcoin shows mixed market signals with current price action.';
