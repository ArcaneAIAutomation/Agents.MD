 ...
 * AI Trade Analyzer
 * 
 * Uses OpenAI GPT-5.1 (Responses API) to analyze completed trades with advanced reasoning
 * Model is configurable via OPENAI_MODEL environment variable
 * Implements fallback to gpt-4.1 if primary model fails
...
export async function analyzeTradeWithAI(
  ...
  try {
    ...
    try {
      console.log(`[ATGE] Analyzing trade with ${MODEL}...`);
...
${prompt}`
      ...
      if (!analysisText) {
        throw new Error(`No analysis generated by ${MODEL}`);
      }
      ...
      console.log(`[ATGE] Trade analysis completed with ${response.model}`);
      return analysis;
    } catch (primaryError) {
      console.error(`[ATGE] ${MODEL} failed, trying ${FALLBACK_MODEL} fallback:`, primaryError);
...
Return ONLY valid JSON with this structure:
{
  ...
  "lessonsLearned": ["string"]
      ...
      // Parse and validate response
      const analysisText = fallbackResponse.content;
      if (!analysisText) {
        throw new Error(`No analysis generated by ${FALLBACK_MODEL}`);
      }
      ...
      // Validate required fields
      if (!analysis.outcome || !analysis.explanation || !analysis.keyFactors) {
        throw new Error(`Invalid analysis structure from ${FALLBACK_MODEL}`);
      }

      console.log(`[ATGE] Trade analysis completed with ${fallbackResponse.model} (fallback)`);
      return analysis;
    }
  } catch (error) {
