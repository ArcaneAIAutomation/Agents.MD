...
async function fetchRealBTCPrice() {
  try {
    ...
    const apis = [
      ...
      `https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd&include_24hr_change=true&include_market_cap=true${keyParam}`,
...
async function fetchCryptoNews() {
  try {
    ...
    const newsApis = [
      `https://newsapi.org/v2/everything?q=bitcoin+BTC+price+analysis&from=${lastTenDays}&sortBy=publishedAt&pageSize=10&language=en&apiKey=${process.env.NEWS_API_KEY}`,
...
export default async function handler(
  ...
  try {
    ...
    if (useStructuredFallback) {
      ...
      const basePrice = realBTCData?.price || 64800; // Updated realistic BTC price for August 2025
      const currentPrice = Math.round(basePrice); // Use exact price without random volatility for consistency
      const rsiValue = 45 + seededRandom(500) * 30; // RSI between 45-75 (consistent)
      const fearGreedIndex = 60 + seededRandom(501) * 25; // Fear & Greed 60-85 (consistent)
      ...
      analysis = {
        ...
        priceAnalysis: {
          ...
          change24h: realBTCData?.change24h || (seededRandom(502) - 0.5) * 5, // Smaller range for realistic changes (consistent)
          support: currentPrice - 2000,
          resistance: currentPrice + 2500
        },
        predictions: {
          hourly: {
            target: Math.round(currentPrice + (seededRandom(503) - 0.5) * 800),
            confidence: realBTCData ? 85 : 70
          },
          daily: {
            target: Math.round(currentPrice + (seededRandom(504) - 0.5) * 2500),
            confidence: realBTCData ? 80 : 65
          },
          weekly: {
            target: Math.round(currentPrice + (seededRandom(505) - 0.5) * 7000),
            confidence: realBTCData ? 75 : 60
          }
        },
        marketData: {
          price: currentPrice,
          change24h: realBTCData?.change24h || (seededRandom(506) - 0.5) * 5,
